        ...
        <div class="span12">
          <div class="portlet box grey" style="border-color:#800000;">
            <div class="portlet-title" style="background-color:#800000;">
              <div class="caption"> <i class="icon-edit"></i> Add New Banner</div>
            </div>
            <div class="portlet-body form">
              <!-- BEGIN FORM-->
              <form name="form" id="form" action="<?php echo base_url('pages/addbanner/');?>" class="form-horizontal" method="post" enctype="multipart/form-data" novalidate="novalidate">
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                    <input type="text" name="title" data-required="0" class="span5 m-wrap">
                  </div>
                </div>                
                <div class="control-group form_field">
                  <label class="control-label">Description </label>
                  <div class="controls">
                    <input type="text" name="description" data-required="0" class="span6 m-wrap">					
                  </div>
                </div>
				<div class="control-group form_field">
                  ...
                  <div class="controls">					
                    ...
                    <input name="post_id" type="hidden" data-required="1">
                    <div style="margin-top:10px">
                        <span class="label label-important">Note :</span>
                        <span style="font-weight:normal">Please Select any one image for banner must be larger then 1024 X 350 ...</span>
                    </div>
                    <button type="submit" class="btn black" style="background-color:#800000;" name="banner_new"><i class="icon-ok"></i> Save</button>
                  </div>
                </div>
                <div class="control-group form_field">
                    <!-- media images-->
                    <?php if(count($media)>0){ $count = 0;?>
                        <div class="control-group">
                        <?php foreach($media as $image){?>
                            <?php //var_dump($image);?>
                            <?php $count++;?>
                            <div class="span2">
                                <div class="media-image" data-postid="<?php echo $image->ID;?>" style="background-image:url(<?php echo base_url('uploads/'.$image->_wp_attached_file);?>);" data-url="<?php echo base_url('uploads/'.$image->_wp_attached_file);?>">
                                    <?php $_wp_attachment_metadata = unserialize($image->_wp_attachment_metadata);?>
                                    <div class="image-diamention"><?php echo $_wp_attachment_metadata['width'].' X '.$_wp_attachment_metadata['height']; ?></div>
                                </div>
                            </div>
...
<script>
		...
		jQuery(document).ready(function() {   
            ...
            jQuery("body").on("click",".media-image",function(){
                $(this).closest(".form_field").find(".media-image").removeClass("active")
                $(this).addClass("active")
                $("input[name=post_id]").val($(this).data('postid'))
                $(".selected-image-for-banner").html('<img src="'+$(this).data("url")+'" />')
            })
