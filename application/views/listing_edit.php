        ...
        <div class="span12">
          <div class="portlet box grey" style="border-color:#800000;">
            <div class="portlet-title" style="background-color:#800000;">
              <div class="caption"> <i class="icon-edit"></i> Edit Listing </div>
            </div>
            <div class="portlet-body form">
              <!-- BEGIN FORM-->
              <form name="listing" id="form" action="<?php echo base_url('listing/update/');?>" class="form-horizontal" method="post" enctype="multipart/form-data" novalidate="novalidate">
                <input name="listing_id" type="hidden" value="<?php echo $listingObj->listing_id; ?>">
                <input name="listing_image" type="hidden" value="<?php echo $listingObj->listing_image; ?>">
                <input type="hidden" name="listing_category" value="<?php //echo $listingObj->listing_category; ?>">
...
<!--           <div class="space20"></div>
                ...
                <div class="control-group form_field">
                  ...
                  <div class="controls">
                    <input type="text" name="listing_seller" value="<?php echo $listingObj->seller_username; ?>" data-required="1" class="span6 m-wrap" style="">
                  </div>
                </div>
                <?php
					$category_sector = $this->Listingdb->category_sector($listingObj->listing_category);
					if(!empty($category_sector)){
						foreach($category_sector as $k=>$category_sectorObj){ $category_sector_id = $category_sectorObj->category_parent; }
					}
				?>
                <!--- FORM SECTION - START : BUSINESS NAME -->
                <?php
				$category_lists = explode("-",$listingObj->listing_category_path);
				//unset($category_lists[0]);
				//unset($category_lists[7]);
				...
				foreach($category_lists as $k=>$listing_category_id){
                            ...
                            <div class="span5">
                                ...
                                <div class="controls">
                                    <div id="listing_sector_1" style="current:block; padding-top:0px">                      
                                        <select name="listing_sector[]" id="listing_sector" class="span12 m-wrap listing_sector" onChange="jQuery.fn.ajax_category_list(this.value,<?php echo $len; ?>);">
                                        <option value="">Select Sector</option>                        
                                        <?php
                                        $sector_list = $this->Listingdb->sector_list();
                                        if(!empty($sector_list)){
                                        foreach($sector_list as $k=>$sectorlistObj){?>                        
                                        <option value="<?php echo $sectorlistObj->category_id; ?>" <?php if($sectorlistObj->category_id == $sector_id){?>selected<?php } ?>><?php echo $sectorlistObj->category_name_1; ?></option>                        
                                        <?php }} ?>
                                        </select>
                                    </div>
                                </div>
					...
					<?php }else{
					$category_id = $listing_category_id;
					?>
                            <div class="span5">
                                <label class="control-label">Category <span class="required">*</span></label>
                                <div class="controls">
                                    <div id="listing_category_<?php echo $len; ?>" class="listing_category" style="current:block; padding-top:0px">
                                        <select name="listing_category[]" id="box_listing_category_1" class="span12 m-wrap" onChange="">
                                    ...
                                    <div id="listing_category_notification" style="display:none; padding-top:5px"> <img src="gui/ltr/img/ajax-loader.gif" align="left" style="padding-right:5px"> <font>please wait while loading sub categories data ...</font> </div>
                                </div>
                            </div>
                        </div>    
               		</div>       
				...
				?>
                <input type="hidden" name="listing_category_count" value="<?php echo ($len-1);?>" />				
                <div class="control-group form_field">
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                    <textarea name="listing_sell_reason" class="span8 m-wrap" rows="6"><?php echo $listingObj->listing_sell_reason; ?></textarea>
                  </div>
                </div>
                ...
                <div class="control-group" style="margin-top:0; margin-bottom:0">
                  <label class="control-label"></label>
                  <div class="controls">
                    <h5 style="font-weight:700">Photo</h5>
                  </div>
                </div>
                <?php //$photoObj = $this->Listingdb->edit_listing_photo($listingObj->listing_id);
				$image=true;
				if(!empty($listingObj->listing_image))
				{
					$url = base_url("photo_big/".$listingObj->listing_image);
					$url_header = @get_headers($url);
					if($url_header[0] == 'HTTP/1.1 404 Not Found' ){
						$url = base_url("photo_big/".$listingObj->listing_image);
						$url_header = @get_headers($url);
						if($url_header[0] == 'HTTP/1.1 404 Not Found' ){$url = base_url("photo_big/sample.jpg");}
					}
					else
					{
						$url = base_url("photo_big/sample.jpg");
					}
				}
				else
				{
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                    <input type="text" name="photo_caption_1" value="<?php echo $listingObj->listing_image;?>" data-required="1" class="span5 m-wrap" style="">
                  </div>
                </div>
                <div class="form-actions">
                  <button type="submit" class="btn black" style="background-color:#800000;" name="listing_update"><i class="icon-ok"></i> Save</button>
                </div>
              </form>
              <!-- END FORM-->
            </div>
...
<script>
		...
		jQuery(document).ready(function() {   
			...
			jQuery("body").on("click","button[name=listing_update]",function(e){
      ...
      */
      ...
      jQuery('body').on("change","form input,form select",function(){
        var slug = $('input[name=listing_id]').val()
        slug +='-'+$('input[name=listing_title_1]').val().replace(/\s/g, '-').replace(/'/g, '-')
